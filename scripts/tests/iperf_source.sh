...
# Usage sudo ./iperf_source.sh $1
# $1 is the output text file of this text which which will be stored in gnuplots/
# Run this on source VM and it runs iperf tcp for different -l options
set -e 
set -o nounset
...
do
  ...
  iperf -c 192.168.20.2 -f b -l $i | tee /dev/tty | grep -m 1 "0.0-10.0" >> temp_bridge_tcp.txt
done
